...
def multi_proxy_minion_process(queue):
    '''
    Start a multi-proxy minion process
    '''
    import salt.cli.daemons
    # salt_minion spawns this function in a new process
    ...
    restart = False
    multiproxyminion = None
    status = salt.defaults.exitcodes.EX_OK
    try:
        multiproxyminion = salt.cli.daemons.MultiProxyMinion()
        multiproxyminion.start()
    except (Exception, SaltClientError, SaltReqTimeoutError, SaltSystemExit) as exc:
        log.error('Multi Proxy Minion failed to start: ', exc_info=True)
        restart = True
        # status is superfluous since the process will be restarted
        status = salt.defaults.exitcodes.SALT_KEEPALIVE
    except SystemExit as exc:
    ...
    if restart is True:
        log.warning('** Restarting multi proxy minion **')
        delay = 60
        if multiproxyminion is not None:
            if hasattr(multiproxyminion, 'config'):
                delay = multiproxyminion.config.get('random_reauth_delay', 60)
        random_delay = randint(1, delay)
        log.info('Sleeping random_reauth_delay of {0} seconds'.format(random_delay))
        # preform delay after minion resources have been cleaned
        queue.put(random_delay)
...
def salt_multi_proxy():
    '''
    Start a multi-proxy minion.
    '''
    import salt.cli.daemons
    import multiprocessing
    if '' in sys.path:
