...
class MultiProxyMinion(parsers.ProxyMinionOptionParser, DaemonsMixin):  # pylint: disable=no-init
    '''
    Create a multi-proxy minion server
    '''

    def _handle_signals(self, signum, sigframe):  # pylint: disable=unused-argument
        # escalate signal to the process manager processes
        self.minion.stop(signum)
        super(MultiProxyMinion, self)._handle_signals(signum, sigframe)
    ...
    def prepare(self):
        ...
        If sub-classed, don't **ever** forget to run:
            ...
            super(YourSubClass, self).prepare()
        '''
        super(MultiProxyMinion, self).prepare()

        if not self.values.proxyid:
            self.error('salt-multi-proxy requires --proxyid')
    ...
    def start(self):
        '''
        Start the actual multi-proxy minion.
        ...
        NOTE: Run any required code before calling `super()`.
        '''
        super(MultiProxyMinion, self).start()
        try:
            if check_user(self.config['user']):
                self.action_log_info('The Multi Proxy Minion is starting up')
                self.verify_hash_type()
                self.minion.tune_in()
                if self.minion.restart:
                    raise SaltClientError('Multi Proxy Minion could not connect to Master')
        except (KeyboardInterrupt, SaltSystemExit) as exc:
            self.action_log_info('Multi Proxy Minion Stopping')
            if isinstance(exc, KeyboardInterrupt):
                log.warning('Exiting on Ctrl-c')
                self.shutdown()
            else:
    ...
    def shutdown(self, exitcode=0, exitmsg=None):
        ...
        :param exitmsg
        '''
        if hasattr(self, 'proxy') and 'proxymodule' in self.minion.opts:
            proxy_fn = self.minion.opts['proxymodule'].loaded_base_name + '.shutdown'
            self.minion.opts['proxymodule'][proxy_fn](self.minion.opts)
        self.action_log_info('Shutting down')
        super(MultiProxyMinion, self).shutdown(
            exitcode, ('The Salt {0} is shutdown. {1}'.format(
                self.__class__.__name__, (exitmsg or '')).strip()))
    # pylint: enable=no-member
