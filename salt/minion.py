...
class Minion(MinionBase):
    ...
    def _handle_decoded_payload(self, data):
        ...
        if self.jid_queue is not None:
            if data['jid'] in self.jid_queue and hasattr(self, 'multiproxy') and not self.multiproxy:
                return
            else:
    ...
    def _handle_payload(self, payload):
        if payload is not None and payload['enc'] == 'aes':
            ...
            if hasattr(self, 'proxy') and hasattr(self, 'multiproxy') and \
                    self.multiproxy:

                ids = self.multiproxy_ids + [self.opts['id']]
                for id in ids:
                    log.debug('Multiproxy handling payload for id {0}'.format(id))
                    self.opts['id'] = id
                    self.opts['grains']['id'] = id
                    self.grains_cache['id'] = id
...
class ProxyMinion(Minion):
    ...
    def _post_master_init(self, master):
        ...
        functions.
        '''
        log.debug('subclassed _post_master_init for standard proxy minion')
...
class MultiProxyMinion(Minion):
    ...
    def _post_master_init(self, master):
        ...
        functions.
        '''
        log.debug("subclassed _post_master_init for a multiproxy")
        ...
        if 'proxy' not in self.opts['pillar'] and 'proxy' not in self.opts:
            errmsg = 'No proxy key found in pillar or opts for id '+self.opts['id']+'. '+ \
                     'Check your pillar/opts configuration and contents.  Salt-multi-proxy aborted.'
            log.error(errmsg)
            self._running = False
            raise SaltSystemExit(code=-1, msg=errmsg)
        ...
        if not self.opts['proxy'].get('multiproxy', False):
            errmsg = 'Salt-multi-proxy started, but proxy config does not have \'multiproxy: True\''
            log.error(errmsg)
            self._running = False
            raise SaltSystemExit(code=-1, msg=errmsg)

        self.multiproxy = True
        fq_proxyname = self.opts['proxy']['proxytype']
        ...
        self.serial = salt.payload.Serial(self.opts)
        self.mod_opts = self._prep_mod_opts()
        self.matcher = MultiMatcher(self.opts, self.functions)
        self.beacons = salt.beacons.Beacon(self.opts, self.functions)
        uid = salt.utils.get_uid(user=self.opts.get('user', None))
        self.proc_dir = get_proc_dir(self.opts['cachedir'], uid=uid)
